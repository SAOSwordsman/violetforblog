    ...
    <section class="container">
      ...
      <section class="c-sort-box">
        <section class="c-s-dl">
          <dl>
            <dt>
              <span class="c-999 fsize14">标签类别</span>
            </dt>
            <dd class="c-s-dl-li">
              <ul class="clearfix">
                <li v-for="(item, index) in tagList" :key="index" :class="{active:oneIndex==index}">
                  <a :title="item.tag" href="#" @click="searchOne(item.tag,index)">{{ item.tag }}</a>
                </li>
              </ul>
            </dd>
          </dl>
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      // 分页切换的方法
      gotoPage(page) {
        tagApi.getBlogByTag(page, 8, this.tagQuery).then(response => {
          this.data = response.data.data
        })
      },
      searchOne(tag,index){
        this.oneIndex = index
        this.tagQuery.tag = tag
        tagApi.getBlogByTag(1, 8, this.tagQuery).then(response => {
          this.data = response.data.data
        })
      }
